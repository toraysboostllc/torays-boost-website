...
--
-- Before any of that: a re-validation gate (step 0, right after begin;)
-- re-checks that the current microsoldering tag set is EXACTLY the 56 known
-- target ids — 56/56 present, 0 missing, 0 extra — the same exact-match
-- check wholesale-catalog-architecture-fix-preflight.sql's own check 5
-- already required moments before this file was authorized to run. This is
-- not redundant: it catches real drift between preflight and migration
-- (RAISE EXCEPTION aborts the whole transaction, nothing partial commits).
--
-- FIVE independent things happen after that gate passes:
--
...
-- ----------------------------------------------------------------------------
-- 0. Safety gate — re-validate, INSIDE this transaction and immediately
--    before the DELETE below, that the current microsoldering tag set is
--    STILL exactly the 56 known target ids (56/56 present, 0 missing, 0
--    extra). wholesale-catalog-architecture-fix-preflight.sql's own check 5
--    already required this moments before this file was authorized to run —
--    this is not redundant: real-world drift between preflight and
--    migration (a manual tag added/removed in between, a second admin
--    session, DESK left open with a stale tab) is exactly the scenario this
--    re-check exists to catch. RAISE EXCEPTION here aborts with a normal
--    Postgres error, which rolls back the ENTIRE transaction (nothing below
--    this point ever runs, nothing partial ever commits) — begin;/commit;
--    already guarantees that; this block only decides WHETHER to let
...
begin
  ...
  if v_tag_id is null then
    raise exception 'catalog_architecture_fix_aborted: microsoldering tag not found — cannot validate the current tag set before retracting it';
  end if;

  select count(*) into v_total_tagged_count from wholesale_service_tags where tag_id = v_tag_id;
  select count(*) into v_matching_count from wholesale_service_tags where tag_id = v_tag_id and service_id = any(v_target_ids);

  if v_matching_count <> 56 then
    raise exception 'catalog_architecture_fix_aborted: expected all 56 known target ids to currently carry the microsoldering tag, found % matching (% missing) — real drift since preflight; investigate before re-running', v_matching_count, (56 - v_matching_count);
  end if;

  if v_total_tagged_count <> 56 then
    raise exception 'catalog_architecture_fix_aborted: expected exactly 56 total microsoldering-tagged relationships, found % (% extra beyond the known 56 target ids) — real drift since preflight; investigate before re-running', v_total_tagged_count, (v_total_tagged_count - 56);
  end if;
end $$;
